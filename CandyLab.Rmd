...
```{r}
library(readr)
candy_data <- read_csv("candy-power-ranking/candy-data.csv")
head(candy_data)
```
...
The sugar percentage is 0.1631(positive log-odd)==>Candy with higher suger percentage suggests that it falls into the chocolate group.
...
The sugar percentage is 1.177154==>For each unit increase in the sugar percentage of the candy, the odds it has caramel flavour increase by 1.177154.
'Whether or not the candy is hard' is -0.4286(negative log-odd) ==> Candy that is hard is unlikely to have chocolate flavour.
...
With the logistic regression model which include 'sugar percentile', 'whether it is caramel' and 'whether it is hard', we are able to guess if the candy falls into the chocolate group approximately 47% of the time.
